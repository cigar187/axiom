"""
KSSI — Strikeouts Suppression Score Index
...
Final: KSSI = clamp(KSSI_base + interaction - volatility, 0, 100)
Projected Ks: projected_ks = base_ks * (1 - 0.25 * ((KSSI - 50) / 50))
"""
...
def compute_gts_modifier(f: "PitcherFeatureSet") -> dict:
    ...
    to the final KSSI score and a (currently informational) IP cap shift.
    ...
    if f.game_total >= 9.5:
        score_adj = (aw - 1.0) * 15.0   # high total → offensive environment → suppress KSSI
        ip_cap_adj = -0.3
    elif f.game_total <= 7.5:
        score_adj = (aw - 1.0) * 15.0   # low total → pitcher-friendly → boost KSSI
        ip_cap_adj = 0.2
    else:
        score_adj = 0.0
        ip_cap_adj = 0.0
...
def score_ocr(f: PitcherFeatureSet) -> float:
    ...
    log.debug("KSSI OCR", pitcher=f.pitcher_name,
...
def score_per(f: PitcherFeatureSet) -> float:
    ...
    log.debug("KSSI PER", pitcher=f.pitcher_name,
...
def score_kop(f: PitcherFeatureSet) -> float:
    ...
    log.debug("KSSI KOP", pitcher=f.pitcher_name,
...
def score_uks(f: PitcherFeatureSet) -> float:
    ...
    log.debug("KSSI UKS", pitcher=f.pitcher_name,
...
def score_tlr(f: PitcherFeatureSet) -> float:
    ...
    log.debug("KSSI TLR", pitcher=f.pitcher_name,
...
def compute_kssi_interaction(
    ...
    """
    Apply KSSI interaction boosts. Returns total boost (capped at 9.0).
    ...
    if ocr > 70 and per_ppa > 65:
        boost += 2.0
        if not silent:
            log.info("KSSI K1 triggered", pitcher=name, ocr=ocr, per_ppa=per_ppa, boost=2.0)

    if pmr_put > 70 and f.relies_on_one_putaway:
        boost += 1.5
        if not silent:
            log.info("KSSI K2 triggered", pitcher=name, pmr_put=pmr_put, boost=1.5)

    lineup_disc = _f(f.lineup_discipline_score)
    if lineup_disc > 65 and per_fps > 60:
        boost += 1.5
        if not silent:
            log.info("KSSI K3 triggered", pitcher=name, lineup_disc=lineup_disc, per_fps=per_fps, boost=1.5)

    if kop_hook > 70 and kop_bpen > 70:
        boost += 1.5
        if not silent:
            log.info("KSSI K4 triggered", pitcher=name, kop_hook=kop_hook, kop_bpen=kop_bpen, boost=1.5)
    ...
    if (
        ...
        boost += 2.0
        if not silent:
            log.info("KSSI K5 triggered", pitcher=name,
                     tlr_top4k=tlr_top4k, k_line=f.k_line, pitcher_median_ks=f.pitcher_median_ks, boost=2.0)

    if kop_tto > 70 and ocr_foul > 65:
        boost += 1.0
        if not silent:
            log.info("KSSI K6 triggered", pitcher=name, kop_tto=kop_tto, ocr_foul=ocr_foul, boost=1.0)

    if uks > 65 and f.weak_edge_command:
        boost += 1.0
        if not silent:
            log.info("KSSI K7 triggered", pitcher=name, uks=uks, weak_edge_command=f.weak_edge_command, boost=1.0)

    # ── Zone Sympathy (Merlin v2.0): same rule as HSSI — umpire zone + pitcher command
    ...
    if uhs_zone > 70 and pcs_cmd > 70:
        boost += 4.0
        if not silent:
            log.info("KSSI Zone Sympathy triggered", pitcher=name,
    ...
    if per_deep > 75 and ocr_disc < 40:
        boost += 4.0
        if not silent:
            log.info("KSSI K8 Swing-and-Miss Collision triggered", pitcher=name,
    ...
    if collision > 70 and vaa is not None and vaa < -5.0:
        boost += 2.5
        if not silent:
            log.info("KSSI K9 Swing Plane Divergence triggered", pitcher=name,
                     collision_score=collision, vaa=vaa, boost=2.5)

    capped = min(boost, 9.0)  # raised cap for new Merlin rules
    if not silent:
        log.info("KSSI interaction total", pitcher=name, raw_boost=boost, capped=capped)
    return capped
...
def compute_kssi_volatility(f: PitcherFeatureSet, silent: bool = False) -> float:
    ...
    if not f.lineup_confirmed:
        penalty += 2.5
        if not silent:
            log.info("KSSI KV1 lineup uncertainty", pitcher=name, penalty=2.5)

    if not f.umpire_confirmed:
        penalty += 1.0
        if not silent:
            log.info("KSSI KV2 umpire unknown", pitcher=name, penalty=1.0)

    if f.per_velo is not None and f.per_velo < 40:
        penalty += 2.0
        if not silent:
            log.info("KSSI KV3 stuff volatility", pitcher=name, per_velo=f.per_velo, penalty=2.0)

    if f.recent_velocity_spike:
        penalty += 1.5
        if not silent:
            log.info("KSSI KV4 velocity spike upside uncertainty", pitcher=name, penalty=1.5)

    if f.key_contact_bats_uncertain:
        penalty += 2.0
        if not silent:
            log.info("KSSI KV5 key contact bats resting", pitcher=name, penalty=2.0)

    if f.kop_bpen is not None and f.kop_bpen < 35:
        penalty += 1.5
        if not silent:
            log.info("KSSI KV6 bullpen depleted", pitcher=name, kop_bpen=f.kop_bpen, penalty=1.5)

    if f.ens_windin is not None and f.ens_windin < 35:
        penalty += 1.5
        if not silent:
            log.info("KSSI KV7 rain/weather timing uncertainty", pitcher=name, ens_windin=f.ens_windin, penalty=1.5)
    ...
    if not silent:
        log.info("KSSI volatility total", pitcher=name, raw_penalty=penalty, capped=capped)
    return capped
...
def kssi_grade(score: float) -> str:
    """
    Grade thresholds anchored to real MLB pitcher distributions — NOT a 0-100 fantasy scale.

    In modern baseball no starter realistically scores 80+ on KSSI. An elite K arm
...
# ─────────────────────────────────────────────────────────────
# Main KSSI computation
# ─────────────────────────────────────────────────────────────

def compute_kssi(f: PitcherFeatureSet, silent: bool = False) -> dict:
    """
    Compute the full KSSI score for one pitcher on one game day.
    ...
    Returns a dict with:
      kssi_base, kssi_interaction, kssi_volatility, kssi, grade,
      projected_ks, base_ks,
      all individual block scores for database logging.

    Backward-compat alias keys (kusi, kusi_base, kusi_interaction, kusi_volatility)
    are also included so existing callers reading those keys continue to work.
    """
    if not silent:
        log.info("KSSI computation starting", pitcher=f.pitcher_name, game_id=f.game_id)
    ...
    if not silent:
        log.info("KSSI base", pitcher=f.pitcher_name, kssi_base=round(kssi_base, 2))

    # ── Interaction boosts (includes Zone Sympathy and K8 from Merlin v2.0)
    interaction = compute_kssi_interaction(
        f=f,
        ocr=ocr,
    ...
    kssi_pre = clamp(kssi_raw)

    # ── Bullpen Fatigue Adjustment
    from app.utils.bullpen import apply_bullpen_to_kusi as apply_bullpen_to_kssi
    kssi = apply_bullpen_to_kssi(kssi_pre, f.bullpen_fatigue_own)
    bullpen_adjustment = round(kssi - kssi_pre, 2)

    # ── Catcher Framing adjustment (SKU #37)
    # catcher_kssi_adj is a local alias for f.catcher_kusi_adj —
    # the feature set field rename is pending a later features.py update.
    kssi_pre_framing = kssi
    catcher_kssi_adj = f.catcher_kusi_adj
    if catcher_kssi_adj != 0.0:
        kssi = kssi * (1.0 + catcher_kssi_adj)
        kssi = round(max(0.0, min(kssi, 100.0)), 2)
        if not silent:
            log.info("KSSI catcher framing adjustment applied",
                     pitcher=f.pitcher_name,
                     catcher=f.catcher_name,
                     strike_rate=f.catcher_strike_rate,
                     framing_label=f.catcher_framing_label,
                     kssi_adj=catcher_kssi_adj,
                     kssi_before=round(kssi_pre_framing, 2),
                     kssi_after=round(kssi, 2))

    # ── Extension / Perceived Velocity boost (SKU #38)
    if f.extension_elite and not silent:
        log.info("KSSI extension elite (perceived velo boost already applied in per_velo)",
                 pitcher=f.pitcher_name,
                 extension_ft=f.extension_ft)

    # ── GTS modifier (game total and market context)
    gts = compute_gts_modifier(f)
    kssi = clamp(kssi + gts["score_adjustment"])
    gts_score_adj = gts["score_adjustment"]
    gts_ip_cap_adj = gts["ip_cap_adjustment"]
    if gts_score_adj != 0.0 and not silent:
        log.info("KSSI GTS modifier applied",
                 pitcher=f.pitcher_name,
                 game_total=f.game_total,
                 score_adjustment=gts_score_adj,
                 ip_cap_adjustment=gts_ip_cap_adj)

    grade = kssi_grade(kssi)
    if not silent:
        log.info("KSSI final",
                 pitcher=f.pitcher_name,
                 kssi_base=round(kssi_base, 2),
                 interaction=round(interaction, 2),
                 volatility=round(volatility, 2),
                 kssi_pre_bullpen=round(kssi_pre, 2),
    ...
    projected_ks = base_ks * (1 - 0.25 * ((kssi - 50) / 50))
    ...
    return {
        ...
        "kssi":             round(kssi, 2),
        "kssi_base":        round(kssi_base, 2),
        "kssi_interaction": round(interaction, 2),
        "kssi_volatility":  round(volatility, 2),
        # ── Backward-compat alias keys — existing callers reading "kusi" continue to work
        "kusi":             round(kssi, 2),
        "kusi_base":        round(kssi_base, 2),
