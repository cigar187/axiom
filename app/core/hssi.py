"""
HSSI — Hits Suppression Score Index
Scoring engine for pitcher hits-allowed under props.

Formula implemented exactly as specified:

  HSSI_base = 0.27*OWC + 0.26*PCS + 0.16*ENS + 0.18*OPS + 0.08*UHS + 0.05*DSC
...
Final: HSSI = clamp(HSSI_base + interaction - volatility, 0, 100)
Projected hits: projected_hits = base_hits * (1 - 0.21 * ((HSSI - 50) / 50))
"""
...
def compute_gts_modifier(f: "PitcherFeatureSet") -> dict:
    """
    GTS — Game Total Score modifier.

    Uses the pre-game over/under line to apply a market-derived adjustment
    to the final HSSI score and a (currently informational) IP cap shift.

    Returns:
        {"score_adjustment": float, "ip_cap_adjustment": float}
    Both default to 0.0 when game_total is None.
    """
    if f.game_total is None or f.game_total == 0.0:
        return {"score_adjustment": 0.0, "ip_cap_adjustment": 0.0}

    aw = f.game_total / 8.5  # attrition weight — 1.0 = neutral

    if f.game_total >= 9.5:
        score_adj = (aw - 1.0) * 15.0   # high total → offensive environment → suppress HSSI
        ip_cap_adj = -0.3
    elif f.game_total <= 7.5:
        score_adj = (aw - 1.0) * 15.0   # low total → pitcher-friendly → boost HSSI
        ip_cap_adj = 0.2
    else:
        score_adj = 0.0
        ip_cap_adj = 0.0
...
def score_owc(f: PitcherFeatureSet) -> float:
    ...
    log.debug("HSSI OWC", pitcher=f.pitcher_name,
...
def score_pcs(f: PitcherFeatureSet) -> float:
    ...
    log.debug("HSSI PCS", pitcher=f.pitcher_name,
...
def score_ens(f: PitcherFeatureSet) -> float:
    ...
    log.debug("HSSI ENS", pitcher=f.pitcher_name,
...
def score_ops(f: PitcherFeatureSet) -> float:
    ...
    log.debug("HSSI OPS", pitcher=f.pitcher_name,
...
def score_uhs(f: PitcherFeatureSet) -> float:
    ...
    log.debug("HSSI UHS", pitcher=f.pitcher_name,
...
def score_dsc(f: PitcherFeatureSet) -> float:
    ...
    log.debug("HSSI DSC", pitcher=f.pitcher_name,
...
def compute_hssi_interaction(
    ...
    """
    Apply HSSI interaction boosts. Returns total boost (capped at 8.0).
    ...
    if pcs_gb > 65 and dsc_infdef > 65:
        boost += 1.5
        if not silent:
            log.info("HSSI H1 triggered", pitcher=name, pcs_gb=pcs_gb, dsc_infdef=dsc_infdef, boost=1.5)

    if fly_supp > 60 and ens_park > 70:
        boost += 1.5
        if not silent:
            log.info("HSSI H2 triggered", pitcher=name, fly_supp=fly_supp, ens_park=ens_park, boost=1.5)

    if ens_windin > 70 and owc_ld > 60:
        boost += 1.0
        if not silent:
            log.info("HSSI H3 triggered", pitcher=name, ens_windin=ens_windin, owc_ld=owc_ld, boost=1.0)

    if ops_hook > 70 and ops_bpen > 70:
        boost += 2.0
        if not silent:
            log.info("HSSI H4 triggered", pitcher=name, ops_hook=ops_hook, ops_bpen=ops_bpen, boost=2.0)

    if owc_bot3 > 70 and ops_tto > 65:
        boost += 1.0
        if not silent:
            log.info("HSSI H5 triggered", pitcher=name, owc_bot3=owc_bot3, ops_tto=ops_tto, boost=1.0)

    if pcs_reg > 65 and pcs_hha > 65:
        boost += 1.0
        if not silent:
            log.info("HSSI H6 triggered", pitcher=name, pcs_reg=pcs_reg, pcs_hha=pcs_hha, boost=1.0)
    ...
    if (
        ...
        boost += 1.5
        if not silent:
            log.info("HSSI H7 triggered", pitcher=name,
    ...
    if uhs_zone > 70 and pcs_cmd > 70:
        boost += 4.0
        if not silent:
            log.info("HSSI Zone Sympathy triggered", pitcher=name,
    ...
    if ens_air > 70 and ens_of > 75:
        boost += 5.0
        if not silent:
            log.info("HSSI E1 Heavy Air triggered", pitcher=name,
                     ens_air=ens_air, ens_of=ens_of, boost=5.0)

    capped = min(boost, 8.0)  # raised cap for new Merlin rules
    if not silent:
        log.info("HSSI interaction total", pitcher=name, raw_boost=boost, capped=capped)
    return capped
...
def compute_hssi_volatility(
    ...
    """
    Apply HSSI volatility penalties. Returns total penalty (capped at 9.5).
    Each triggered penalty is logged individually.

    Merlin v2.0 addition:
      H8 Pressure Cooker: OPS_TRAFFIC > 75 AND OWC_BOT3 < 40 → -3.5 HSSI Penalty.
    ...
    if not f.lineup_confirmed:
        penalty += 2.5
        if not silent:
            log.info("HSSI HV1 lineup uncertainty", pitcher=name, penalty=2.5)

    # HV2: weather uncertainty — treat ens_windin < 40 as meaningful weather risk
    if f.ens_windin is not None and f.ens_windin < 40:
        penalty += 1.5
        if not silent:
            log.info("HSSI HV2 weather uncertainty", pitcher=name, ens_windin=f.ens_windin, penalty=1.5)

    if f.babip_variance_high:
        penalty += 1.5
        if not silent:
            log.info("HSSI HV3 extreme BABIP variance", pitcher=name, penalty=1.5)

    # HV4: poor defense — dsc_def < 40 means below-average defense
    if f.dsc_def is not None and f.dsc_def < 40:
        penalty += 2.0
        if not silent:
            log.info("HSSI HV4 poor defense", pitcher=name, dsc_def=f.dsc_def, penalty=2.0)

    # HV5: backup catcher — dsc_catch < 35
    if f.dsc_catch is not None and f.dsc_catch < 35:
        penalty += 1.0
        if not silent:
            log.info("HSSI HV5 backup catcher downgrade", pitcher=name, dsc_catch=f.dsc_catch, penalty=1.0)

    if not f.umpire_confirmed:
        penalty += 0.8
        if not silent:
            log.info("HSSI HV6 umpire unknown", pitcher=name, penalty=0.8)

    if pcs_consistency < 40:
        penalty += 2.0
        if not silent:
            log.info("HSSI HV7 low consistency", pitcher=name, pcs_consistency=pcs_consistency, penalty=2.0)

    # HV8: bullpen depleted — ops_bpen < 35
    if f.ops_bpen is not None and f.ops_bpen < 35:
        penalty += 1.2
        if not silent:
            log.info("HSSI HV8 bullpen depleted", pitcher=name, ops_bpen=f.ops_bpen, penalty=1.2)
    ...
    if f.park_extreme:
        ...
        if not silent:
            ...
            log.info("HSSI HV9 extreme hitter park", pitcher=name,
    ...
    if f.hard_hit_tier == "STRUGGLING":
        penalty += 1.5
        if not silent:
            log.info("HSSI HV10 elevated hard-hit rate", pitcher=name,
                     hard_hit_pct=f.season_hard_hit_pct, tier="STRUGGLING", penalty=1.5)
    elif f.hard_hit_tier == "DISASTER":
        penalty += 3.0
        if not silent:
            log.info("HSSI HV10 disaster hard-hit rate", pitcher=name,
    ...
    # This is logged but applied as a negative penalty (net boost to HSSI)
    elif f.hard_hit_tier == "ELITE":
        penalty -= 2.0   # reduces total penalty = net HSSI boost
        if not silent:
            log.info("HSSI HV10 elite hard-hit suppression BOOST", pitcher=name,
    ...
    if ops_traffic > 75 and owc_bot3 < 40:
        penalty += 3.5
        if not silent:
            log.info("HSSI H8 Pressure Cooker triggered", pitcher=name,
                     ops_traffic=ops_traffic, owc_bot3=owc_bot3, penalty=3.5)

    # Cap: 9.5 max penalty, no floor — negative penalty = net HSSI boost (ELITE hard-hit tier)
    # When ELITE fires: penalty -= 2.0, resulting in a net 2-point HSSI boost via the subtraction
    # in compute_hssi: hssi_raw = hssi_base + interaction - volatility
    capped = min(penalty, 9.5)
    if not silent:
        log.info("HSSI volatility total", pitcher=name, raw_penalty=penalty, capped=capped)
    return capped
...
def hssi_grade(score: float) -> str:
    """
    Grade thresholds anchored to real MLB pitcher distributions — NOT a 0-100 fantasy scale.

    In modern baseball no starter realistically scores 85+ on HSSI. An ace in a
...
# ─────────────────────────────────────────────────────────────
# Main HSSI computation
# ─────────────────────────────────────────────────────────────

def compute_hssi(f: PitcherFeatureSet, silent: bool = False) -> dict:
    """
    Compute the full HSSI score for one pitcher on one game day.
    ...
    Returns a dict with:
      hssi_base, hssi_interaction, hssi_volatility, hssi, grade,
      projected_hits, base_hits,
      all individual block scores for database logging.

    Backward-compat alias keys (husi, husi_base, husi_interaction, husi_volatility)
    are also included so existing callers reading those keys continue to work.
    """
    if not silent:
        log.info("HSSI computation starting", pitcher=f.pitcher_name, game_id=f.game_id)
    ...
    if not silent:
        log.info("HSSI base", pitcher=f.pitcher_name, hssi_base=round(hssi_base, 2))

    # ── Interaction boosts (includes Zone Sympathy and E1 from Merlin v2.0)
    fly_supp = _f(f.fly_ball_suppression)
    interaction = compute_hssi_interaction(
        f=f,
        pcs_gb=_f(f.pcs_gb),
    ...
    volatility = compute_hssi_volatility(
        f,
    ...
    # ── Final HSSI (pre-bullpen)
    hssi_raw = hssi_base + interaction - volatility
    hssi_pre = clamp(hssi_raw)

    # ── Bullpen Fatigue Adjustment
    # Tired opponent bullpen = more hit opportunities = HSSI goes UP.
    # Formula: Final_HSSI = Base_HSSI × (1 + BFS_opponent)
    from app.utils.bullpen import apply_bullpen_to_husi as apply_bullpen_to_hssi
    hssi = apply_bullpen_to_hssi(hssi_pre, f.bullpen_fatigue_opp)
    bullpen_adjustment = round(hssi - hssi_pre, 2)

    # ── Travel & Fatigue Index adjustment (SKU #14)
    # A tired/traveling pitching team gives up more hits → HSSI goes DOWN.
    # Formula: HSSI_tfi = HSSI × (1 - tfi_penalty_pct)
    from app.utils.travel_fatigue import apply_tfi_to_husi as apply_tfi_to_hssi
    hssi_pre_tfi = hssi
    hssi = apply_tfi_to_hssi(hssi, f.tfi_penalty_pct)
    tfi_adjustment = round(hssi - hssi_pre_tfi, 2)
    if not silent and f.tfi_penalty_pct > 0:
        log.info("HSSI TFI penalty applied",
                 pitcher=f.pitcher_name,
                 tfi_label=f.tfi_label,
                 rest_hours=f.tfi_rest_hours,
                 tz_shift=f.tfi_tz_shift,
                 penalty_pct=f.tfi_penalty_pct,
                 hssi_before=round(hssi_pre_tfi, 2),
                 hssi_after=round(hssi, 2))

    # ── GTS modifier (game total and market context)
    gts = compute_gts_modifier(f)
    hssi = clamp(hssi + gts["score_adjustment"])
    gts_score_adj = gts["score_adjustment"]
    gts_ip_cap_adj = gts["ip_cap_adjustment"]
    if gts_score_adj != 0.0 and not silent:
        log.info("HSSI GTS modifier applied",
                 pitcher=f.pitcher_name,
                 game_total=f.game_total,
                 score_adjustment=gts_score_adj,
                 ip_cap_adjustment=gts_ip_cap_adj)

    grade = hssi_grade(hssi)
    if not silent:
        log.info("HSSI final",
                 pitcher=f.pitcher_name,
                 hssi_base=round(hssi_base, 2),
                 interaction=round(interaction, 2),
                 volatility=round(volatility, 2),
                 hssi_pre_bullpen=round(hssi_pre, 2),
    ...
    projected_hits = base_hits * (1 - 0.21 * ((hssi - 50) / 50))
    ...
    if f.vaa_flat and f.vaa_contact_penalty > 0:
        ...
        if (
            ...
            if not silent:
                log.info("HSSI VAA Elevation BOOST applied (Merlin override)",
                         pitcher=f.pitcher_name,
            ...
            projected_hits = projected_hits * (1.0 + f.vaa_contact_penalty)
            if not silent:
                log.info("HSSI VAA flat penalty applied",
                         pitcher=f.pitcher_name,
                         vaa_degrees=f.vaa_degrees,
                         pitch_location_high_pct=f.pitch_location_high_pct,
    ...
    if f.season_gb_pct is not None:
        ...
        if gb_mult != 1.0:
            ...
            if not silent:
                log.info("HSSI GB suppressor applied",
                         pitcher=f.pitcher_name,
                         gb_value=gb_val,
                         scale=scale_label,
    ...
    if f.park_hits_multiplier is not None and f.park_hits_multiplier != 1.0:
        ...
        if not silent:
            log.info("HSSI park factor override applied",
                     pitcher=f.pitcher_name,
                     park_score=round(f.ens_park or 50.0, 1),
    ...
    if f.tbapi_hits_mult != 1.0:
        ...
        if not silent:
            log.info("HSSI TBAPI modifier applied",
                     pitcher=f.pitcher_name,
                     tbapi=f.tbapi,
    ...
    if f.fi_hits_mult != 1.0:
        ...
        if not silent:
            log.info("HSSI Fragility Index modifier applied",
                     pitcher=f.pitcher_name,
                     fi_tier=f.fi_tier,
                     fi_score=f.fi_score,
    ...
    return {
        ...
        "hssi":             round(hssi, 2),
        "hssi_base":        round(hssi_base, 2),
        "hssi_interaction": round(interaction, 2),
        "hssi_volatility":  round(volatility, 2),
        # ── Backward-compat alias keys — existing callers reading "husi" continue to work
        "husi":             round(hssi, 2),
        "husi_base":        round(hssi_base, 2),
